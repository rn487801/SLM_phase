...
def safe_move(stage, angle, timeout=3.0):
    """Move a Thorlabs ELLx stage with a manual timeout to avoid hanging forever."""
    stage.move_to(angle, blocking=False)
    t0 = time.time()

    # Poll until motion stops, or timeout occurs
    while stage.stage.is_moving():
        time.sleep(0.01)
        if time.time() - t0 > timeout:
            print(f"[WARNING] Timeout when moving to {angle:.1f}°. Continuing anyway.")
            return False
    return True
...
def measure_polarized_spectra(
    ...
    for i_qwp, qwp_angle in enumerate(qwp_angles):
        ...
        qwp.home(blocking=True)
        polarizer.home(blocking=True)
        safe_move(qwp, qwp_angle)

        for i_pol, pol_angle in enumerate(pol_angles):
            safe_move(polarizer, pol_angle)
